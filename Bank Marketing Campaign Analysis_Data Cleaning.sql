...
SELECT 
  ...
  SUM(CASE WHEN poutcome IS NULL  THEN 1 ELSE 0 END) AS null_poutcome
FROM `bank-business-performance.bank_marketing.bank_clients`;

--4.Check duplicates
SELECT age, job, marital, education,`default`, balance, housing, loan, contact, day, month, duration,campaign, pdays, previous, poutcome, deposit, COUNT(*) AS cnt
FROM `bank-business-performance.bank_marketing.bank_clients`
...
--Double check
SELECT DISTINCT * FROM `bank-business-performance.bank_marketing.bank_clients`;
